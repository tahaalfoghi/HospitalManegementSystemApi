...
namespace HospitalManegementSystem.Controllers
{
    ...
    public class WardController:ControllerBase
    {
        ...
        public WardController(IRepository<Ward> repo, ILogger<WardController> logger, IMapper mapper)
        {
            _repo = repo;
            _logger = logger;
            _mapper = mapper;
        ...
        public async Task<ActionResult<IEnumerable<WardDTO>>> GetAll()
        {
            try
            {
                var records = await _repo.GetAllAsync();
                var recordsDTO = _mapper.Map<HashSet<WardDTO>>(records);
                return recordsDTO;
            }
            catch (Exception e)
            {
        ...
        [ProducesResponseType(404)]
        [ProducesResponseType(500)]
        public async Task<ActionResult<WardDTO>> GetById([FromBody] int id)
        {
            try
            {
                var record = await _repo.GetByIdAsync(id);

                var recordDTO = _mapper.Map<WardDTO>(record);
                return recordDTO;
            }
            catch (Exception e)
            {
        ...
        [ProducesResponseType(404)]
        [ProducesResponseType(500)]
        public async Task<ActionResult> Create([FromBody] WardDTO dto)
        {
            ...
            var record = _mapper.Map<Ward>(dto);
        ...
        [ProducesResponseType(404)]
        [ProducesResponseType(500)]
        public async Task<ActionResult> Update(int id, WardDTO dto)
        {
            ...
            await _repo.UpdateAsync(id, ward);
